# Hygen Development with DF Devspaces
...
```bash
cd ..
devspaces bind hygen
```
4. Grab some container info

```bash
devspaces info hygen
```

5. Connect to development container

```bash
devspaces exec hygen
```
...
hygen init self
```
Now you should see files generated by hygen in `test` directory. 


## Running hygen via Docker-Compose file
...
### Dockerfile
 Dockefile is created on top of `node:11-stretch` image.

### Requirements
 - The project should be cloned from https://github.com/trilogy-group/hygen.git
...
- At this point you must be inside the docker container. From here you can continue from section `6. Install hygen`
